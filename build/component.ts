...
const buildEachComponent = async () => {
  ...
  const builds = files.map(async (file: string) => {
    ...
    const config = {
      ...
      external: (id) => /^vue/.test(id) || /^@lilt-ui-vue/.test(id),
    };
    const bundle = await rollup(config);
    const options = Object.values(buildConfig).map((config) => ({
      format: config.format,
      file: path.resolve(config.output.path, `components/${file}/index.js`),
      paths: pathRewriter(config.output.name), // @lilt-ui-vue => lilt-ui-vue/es  lilt-ui-vue/lib
    }));
...
async function genTypes() {
  ...
  await Promise.all(
    filePaths.map(async function (file) {
      if (file.endsWith(".vue")) {
        ...
        if (script) {
          let content = script.content; // 拿到脚本  button.vue.ts  => button.vue.d.ts
  ...
  const tasks = sourceFiles.map(async (sourceFile: any) => {
    ...
    const tasks = emitOutput.getOutputFiles().map(async (outputFile: any) => {
      ...
      // @lilt-ui-vue -> lilt-ui-vue/es -> .d.ts 肯定不用去lib下查找
