...
function Star(this: any, x, y,color) {
  ...
  this.upSpeedX = (Math.random() * (props.diffusion * 10)) * Math.pow(-1, Math.round(Math.random()))
}
...
const create = (element) => {
  ...
  for (let i = 0 ; i < props.quantity;i++ ) {
    stars.push(new Star(elementLocation.x, elementLocation.y,colorArray[Math.floor((Math.random() * colorArray.length))]))
  }
  // 控制星星出现动画的时间
  let appearTime = 0
  timer = setInterval(() => {
